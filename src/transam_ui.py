...
#
# This gradio ui code is modified from gradio_ui.py (https://github.com/huggingface/smolagents/blob/main/src/smolagents/gradio_ui.py) by Ippei Akiya.
import os
import re
import shutil
import yaml
...
def _process_final_answer_step(step_log: FinalAnswerStep) -> Generator:
    ...
    if isinstance(final_answer, AgentText):
        ...
        # Save final answer
        final_answer_dict = yaml.safe_load(final_answer)
        concept_id = final_answer_dict['conceptId']

        with open(f"{os.environ['TRANSAM_WORK_DIR']}/output/biomedical_concept_{concept_id}.yaml", "w", encoding="utf-8") as f:
        ...
        with gr.Blocks(theme="ocean", fill_height=True) as demo:
            ...
            with gr.Row():
                with gr.Column():
                    gr.Markdown(
                        f"# {self.name}"
                        "\n> Agentic AI for Curating CDISC Biomedical Concepts"
                    )

            with gr.Row():
                with gr.Column():
                    gr.Markdown("**Instruction**", container=False)
